...
function toggleDone(b) {
  const index = demoarray.findIndex((myitem) => myitem.id === Number(b));// find the index of the todo with the specified ID in the demoarray
  demoarray[index].checked = !demoarray[index].checked;  // toggle the checked status of the todo
  renderTodo(demoarray[index]); // render the updated todo in the UI
}
...
function deleteTodo(c) {
  ...
  demoarray = demoarray.filter((myitem) => myitem.id !== Number(c)); // filter out the deleted todo from the demoarray
